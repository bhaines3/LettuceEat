...
describe("LettuceEat", function() {
    ...
    Nightmare({ show: true })
        .goto("https://lettuceeat.herokuapp.com/")
        .type("#name-login", "ernestosGrill@email.com")
        .type("#password_login", "123456")
        .click("#login-user")
        .wait(5000)
        .evaluate(function() {
            return document.querySelector(".display-4 ").innerText;
        })
        .then(function(text) {
            expect(text).to.equal("Ernesto's Grill");
            done();
        });
    });
